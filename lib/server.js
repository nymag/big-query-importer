...
const _ = require('lodash'),
  ...
  sites = ['thejob', 'betamale', 'nyxny', 'selectall', 'strategist', 'vindicated', 'bestofny',  'scienceofus', 'thecut', 'daily/intelligencer'];
    ...
    data.forEach(function (uri) {
      ...
      .then(results => {
        let articleData = _.pick(results.main[0], ['date', 'canonicalUrl', 'primaryHeadline', 'seoHeadline', 'overrideHeadline', 'shortHeadline', 'syndicatedUrl', 'featureTypes', 'tags', 'contentChannel', 'authors', 'rubric', 'cmsSource']),
          headLayoutData = _.pick(results.headLayout[0], ['twitterTitle', 'ogTitle', 'clayType']);

        Object.assign(obj, articleData, headLayoutData)
        obj.pageUri = uri;
        if (obj.authors) {
