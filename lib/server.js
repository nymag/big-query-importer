...
const _ = require('lodash'),
  ...
  sites = ['thejob, betamale, strategist, selectall, nyxny, bestofny, vindicated, daily/intelligencer, thecut'];
...
const pages = sites.map(site => {
  ...
  .catch(logError)
  .then(data => {
    data.forEach(function (uri, index) {
      let obj = {},
        options = {
      ...
      .then(json => JSON.parse(json))
      .then(results => {
        obj = _.pick(results.main[0], ['date', 'canonicalUrl', 'primaryHeadline', 'seoHeadline', 'overrideHeadline', 'shortHeadline', 'featureTypes', 'tags', 'contentChannel', 'authors'])
        obj.pageUri = uri;
        if (obj.authors) {
