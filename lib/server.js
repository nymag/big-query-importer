...
const pages = sites.map(site => {
  ...
  .then(data => {
    data.forEach(function (uri) {
      ...
      .then(json => JSON.parse(json))
      .then(results => {
        obj = _.pick(results.main[0], ['date', 'canonicalUrl', 'primaryHeadline', 'seoHeadline', 'overrideHeadline', 'shortHeadline', 'featureTypes', 'tags', 'contentChannel', 'authors', 'rubric', 'cmsSource'])
        obj.pageUri = uri;
        if (obj.authors) {
